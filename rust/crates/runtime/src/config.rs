...
pub const BREWCODE_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
/// OAuth client configuration used by the main Brewcode runtime.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".brewcode.json"),
            |parent| parent.join(".brewcode.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".brewcode.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".brewcode").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".brewcode").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("BREWCODE_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".brewcode")))
        .unwrap_or_else(|| PathBuf::from(".brewcode"))
}
...
mod tests {
    use super::{
        ...
        RuntimeHookConfig, RuntimePluginConfig, BREWCODE_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn loads_and_merges_claude_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".brewcode");
        fs::create_dir_all(cwd.join(".brewcode")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".brewcode.json"),
        ...
        assert_eq!(BREWCODE_SETTINGS_SCHEMA_NAME, "SettingsSchema");
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".brewcode");
        let project_settings = cwd.join(".brewcode").join("settings.json");
        fs::create_dir_all(cwd.join(".brewcode")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
