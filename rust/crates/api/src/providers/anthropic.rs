...
fn enrich_bearer_auth_error(error: ApiError, auth: &AuthSource) -> ApiError {
    let ApiError::Api(mut details) = error else {
        return error;
    };
    if details.status.as_u16() != 401 {
        return ApiError::Api(details);
    }
    ...
    details.message = Some(match details.message.take() {
        Some(existing) => format!("{existing} — hint: {SK_ANT_BEARER_HINT}"),
        None => format!("hint: {SK_ANT_BEARER_HINT}"),
    });
    ApiError::Api(details)
}
...
mod tests {
    ...
    fn enrich_bearer_auth_error_appends_sk_ant_hint_on_401_with_pure_bearer_token() {
        ...
        let error = crate::error::ApiError::Api(Box::new(ApiErrorDetails {
        ...
        match enriched {
            crate::error::ApiError::Api(details) => {
                assert_eq!(details.status, reqwest::StatusCode::UNAUTHORIZED);
            }
