...
const SLASH_COMMAND_SPECS: &[SlashCommandSpec] = &[
    ...
    SlashCommandSpec {
        ...
        summary: "Manage Brew Code plugins",
        argument_hint: Some(
...
impl DefinitionSource {
    fn report_scope(self) -> DefinitionScope {
        match self {
            Self::ProjectBrewcode | Self::ProjectCodex | Self::ProjectClaude => {
                DefinitionScope::Project
            }
            Self::UserBrewcodeConfigHome | Self::UserCodexHome => DefinitionScope::UserConfigHome,
            Self::UserBrewcode | Self::UserCodex | Self::UserClaude => DefinitionScope::UserHome,
        }
    }
    ...
    match normalize_optional_args(args) {
        ...
        Some(args) if args.split_whitespace().next() == Some("list") && args.contains(' ') => {
            ...
            Ok(render_mcp_unsupported_action_text(
                args,
                "list accepts no filter argument; use `brewcode mcp list`",
            ))
        }
        Some(args) if matches!(args.split_whitespace().next(), Some("info" | "describe")) => {
            Ok(render_mcp_unsupported_action_text(
                args,
                "use `brewcode mcp show <server>` to inspect a server",
            ))
        }
...
fn render_mcp_unsupported_action_json(action: &str, hint: &str) -> Value {
    json!({
        ...
        "usage": {
            ...
            "direct_cli": "brewcode mcp [list|show <server>|help]",
        },
    })
}
...
fn discover_definition_roots(cwd: &Path, leaf: &str) -> Vec<(DefinitionSource, PathBuf)> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_root(
            &mut roots,
            DefinitionSource::ProjectBrewcode,
            ancestor.join(".brewcode").join(leaf),
        );
        push_unique_root(
            &mut roots,
    ...
    if let Ok(brewcode_config_home) = env::var("BREWCODE_CONFIG_HOME") {
        push_unique_root(
            &mut roots,
            DefinitionSource::UserBrewcodeConfigHome,
            PathBuf::from(brewcode_config_home).join(leaf),
        );
    }
...
fn discover_skill_roots(cwd: &Path) -> Vec<SkillRoot> {
    ...
    for ancestor in cwd.ancestors() {
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectBrewcode,
            ancestor.join(".brewcode").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::ProjectBrewcode,
            ancestor.join(".brewcode").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
    ...
    if let Ok(brewcode_config_home) = env::var("BREWCODE_CONFIG_HOME") {
        let brewcode_config_home = PathBuf::from(brewcode_config_home);
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserBrewcodeConfigHome,
            brewcode_config_home.join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserBrewcodeConfigHome,
            brewcode_config_home.join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
    }
    ...
    if let Some(home) = env::var_os("HOME") {
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserBrewcode,
            home.join(".brewcode").join("skills"),
            SkillOrigin::SkillsDir,
        );
        push_unique_skill_root(
            &mut roots,
        ...
        push_unique_skill_root(
            &mut roots,
            DefinitionSource::UserBrewcode,
            home.join(".brewcode").join("commands"),
            SkillOrigin::LegacyCommandsDir,
        );
        push_unique_skill_root(
            &mut roots,
...
fn default_skill_install_root() -> std::io::Result<PathBuf> {
    if let Ok(brewcode_config_home) = env::var("BREWCODE_CONFIG_HOME") {
        return Ok(PathBuf::from(brewcode_config_home).join("skills"));
    }
    if let Ok(codex_home) = env::var("CODEX_HOME") {
        return Ok(PathBuf::from(codex_home).join("skills"));
    }
    if let Some(home) = env::var_os("HOME") {
        return Ok(PathBuf::from(home).join(".brewcode").join("skills"));
    }
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        "unable to resolve a skills install root; set BREWCODE_CONFIG_HOME or HOME",
    ))
}
...
fn render_skills_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       brewcode skills [list|install <path>|help|<skill> [args]]".to_string(),
        "  Invoke           /skills help overview -> $help overview".to_string(),
        "  Install root     $BREWCODE_CONFIG_HOME/skills or ~/.brewcode/skills".to_string(),
        "  Sources          .brewcode/skills, .omc/skills, .agents/skills, .codex/skills, .claude/skills, ~/.brewcode/skills, ~/.omc/skills, ~/.claude/skills/omc-learned, ~/.codex/skills, ~/.claude/skills, legacy /commands".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_skills_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            ...
            "aliases": ["/skill"],
            "direct_cli": "brewcode skills [list|install <path>|help|<skill> [args]]",
            "invoke": "/skills help overview -> $help overview",
            "install_root": "$BREWCODE_CONFIG_HOME/skills or ~/.brewcode/skills",
            "sources": [
                ".brewcode/skills",
                ".omc/skills",
                ".agents/skills",
...
fn render_mcp_usage(unexpected: Option<&str>) -> String {
    let mut lines = vec![
        ...
        "  Direct CLI       brewcode mcp [list|show <server>|help]".to_string(),
        "  Sources          .brewcode/settings.json, .brewcode/settings.local.json".to_string(),
    ];
    if let Some(args) = unexpected {
        lines.push(format!("  Unexpected       {args}"));
    }
...
fn render_mcp_usage_json(unexpected: Option<&str>) -> Value {
    json!({
        ...
        "usage": {
            "slash_command": "/mcp [list|show <server>|help]",
            "direct_cli": "brewcode mcp [list|show <server>|help]",
            "sources": [".brewcode/settings.json", ".brewcode/settings.local.json"],
        },
        "unexpected": unexpected,
    })
}
...
fn definition_source_id(source: DefinitionSource) -> &'static str {
    match source {
        DefinitionSource::ProjectBrewcode
        | DefinitionSource::ProjectCodex
        | DefinitionSource::ProjectClaude => "project_brewcode",
...
mod tests {
    ...
    fn skills_show_and_list_filter_do_not_invoke_model() {
        ...
        // Regression for: `brewcode skills show plan` unexpectedly spawned a model session.
    ...
    fn renders_per_command_help_detail() {
        ...
        assert!(help.contains("Summary          Manage Brew Code plugins"));
        assert!(help.contains("Aliases          /plugins, /marketplace"));
    ...
    fn renders_agents_reports_as_json() {
        ...
        assert_eq!(report["agents"][2]["shadowed_by"]["id"], "project_brewcode");

        let help = handle_agents_slash_command_json(Some("help"), &workspace).expect("agents help");
        assert_eq!(help["kind"], "agents");
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["direct_cli"], "brewcode agents [list|help]");
    ...
    fn resolves_project_skills_and_legacy_commands_from_shared_registry() {
        ...
        let project_skills = workspace.join(".brewcode").join("skills");
        let legacy_commands = workspace.join(".brewcode").join("commands");
    ...
    fn renders_skills_reports_as_json() {
        ...
        assert_eq!(report["skills"][0]["source"]["id"], "project_brewcode");
        assert_eq!(report["skills"][1]["name"], "deploy");
        assert_eq!(report["skills"][1]["origin"]["id"], "legacy_commands_dir");
        assert_eq!(report["skills"][3]["shadowed_by"]["id"], "project_brewcode");
        ...
        assert_eq!(
            ...
            "brewcode skills [list|install <path>|help|<skill> [args]]"
        );
    ...
    fn mcp_usage_supports_help_and_unexpected_args() {
        ...
        assert!(help.contains("Direct CLI       brewcode mcp [list|show <server>|help]"));
    ...
    fn renders_mcp_reports_from_loaded_config() {
        ...
        fs::create_dir_all(workspace.join(".brewcode")).expect("workspace config dir");
        fs::create_dir_all(&config_home).expect("config home");
        fs::write(
            workspace.join(".brewcode").join("settings.json"),
            r#"{
              "mcpServers": {
                "alpha": {
                  "command": "uvx",
        ...
        fs::write(
            workspace.join(".brewcode").join("settings.local.json"),
            r#"{
              "mcpServers": {
                "remote": {
                  "type": "ws",
    ...
    fn renders_mcp_reports_as_json() {
        ...
        assert_eq!(help["action"], "help");
        assert_eq!(help["usage"]["sources"][0], ".brewcode/settings.json");
    ...
    fn mcp_degrades_gracefully_on_malformed_mcp_config_144() {
        // #144: mirror of #143's partial-success contract for `brewcode mcp`.
        // Previously `mcp` hard-failed on any config parse error, hiding
        // well-formed servers and forcing brewcodes to fall back to `doctor`.
        ...
        fs::create_dir_all(workspace.join(".brewcode")).expect("create workspace .brewcode dir");
