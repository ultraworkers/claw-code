...
use runtime::{
    check_base_commit, check_lsp_availability, command_exists_on_path, discover_available_servers,
    format_install_prompt, format_stale_base_warning, format_usd, known_lsp_servers,
    ...
    BaseCommitState, CompactionConfig, ConfigFileReport, ConfigInspection, ConfigLoader,
    ConfigSource, ContentBlock, ContextFile, ConversationMessage, ConversationRuntime,
    LspInstallAction, LspServerDescriptor, McpConfigCollection, McpInvalidServerConfig, McpServer,
        ...
        match action {
            None | Some("status") => {
                ...
                let available = actions
                    .iter()
                    .filter(|a| matches!(a, LspInstallAction::Installed))
                    .count();
            ...
            Some("list") => {
                ...
                for server in &servers {
                    ...
                    println!(
                        "  {status} {:<25} {:<20} — {exts}",
                        server.language, server.command
                    );
                }
            }
            Some("start") | Some("stop") => {
                println!(
                    "LSP servers are managed automatically via lazy-start when tools need them."
                );
