...
fn resumed_binary_accepts_slash_commands_with_arguments() {
    ...
    assert!(stdout.contains("Resume previous  brewcode --resume"));
...
fn resumed_config_command_loads_settings_files_end_to_end() {
    ...
    fs::create_dir_all(project_dir.join(".brewcode")).expect("project config dir should exist");
    ...
    fs::write(
        project_dir.join(".brewcode").join("settings.local.json"),
        r#"{"model":"opus"}"#,
    )
    .expect("local config should write");

    // when
    let output = run_brewcode_with_env(
        &project_dir,
        &[
            "--resume",
            session_path.to_str().expect("utf8 path"),
        ...
        &[("BREWCODE_CONFIG_HOME", config_home.to_str().expect("utf8 path"))],
    );
...
fn resume_latest_restores_the_most_recent_managed_session() {
    ...
    // when
    let output = run_brewcode(&project_dir, &["--resume", "latest", "/status"]);
...
fn resumed_status_command_emits_structured_json_when_requested() {
    ...
    // when
    // Use an isolated BREWCODE_CONFIG_HOME so ~/.brewcode/settings.json is not loaded,
...
fn run_brewcode(current_dir: &Path, args: &[&str]) -> Output {
    run_brewcode_with_env(current_dir, args, &[])
}
...
fn run_brewcode_with_env(current_dir: &Path, args: &[&str], envs: &[(&str, &str)]) -> Output {
