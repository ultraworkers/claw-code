...
383. **Top-level `permissions --help --output-format json` hangs with zero stdout/stderr instead of returning bounded command help JSON** — dogfooded 2026-04-30 for the 04:00 nudge on current `origin/main` / local workspace while the checked-out `./rust/target/debug/claw` binary still reported embedded `git_sha` `d95b230c`. After verifying adjacent help-surface hangs for `cost`, `tokens`, `cache`, and `model`, a fresh probe on the permissions/control-plane surface showed the same silent failure class: `timeout --kill-after=1s 8s ./rust/target/debug/claw permissions --help --output-format json` exited `124` with `stdout=0` and `stderr=0`. In the same workspace, `version --output-format json` returned promptly with version/build metadata, proving the binary itself and JSON output path are reachable. This is distinct from permission-mode execution semantics: the affected surface is help/discovery for the top-level `permissions` command, where agents need bounded local usage/options before inspecting or changing permission policy. **Required fix shape:** (a) make `permissions --help --output-format json` return static/bounded stdout JSON with `kind:"help"` or `kind:"permissions"`, `action:"help"`, usage, options, examples, supported output formats, and related slash/direct commands; (b) ensure help rendering does not initialize slow permission/session/provider state; (c) if any dynamic provider is consulted, return a typed JSON timeout/unavailable error instead of hanging; (d) add regression coverage proving permissions help in JSON mode returns within a deterministic budget. **Why this matters:** permissions are a safety-critical control-plane surface. If permissions help hangs silently, claws cannot safely discover how to inspect or adjust permission mode before execution, and automation stalls before it can choose a safe policy path. Source: gaebal-gajae dogfood follow-up for the 04:00 nudge on rebuilt/local `./rust/target/debug/claw` `d95b230c`.
