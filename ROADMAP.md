...
347. **Top-level `mcp show <missing-server> --output-format json` hangs with zero stdout/stderr instead of returning a typed not-found error** — dogfooded 2026-04-29 for the 20:30 nudge on current `origin/main` / rebuilt `./rust/target/debug/claw` with embedded `git_sha` `ee41b266`. After rebuilding and verifying `./rust/target/debug/claw version --output-format json` reported `git_sha` `ee41b266`, running `timeout 15 ./rust/target/debug/claw mcp show does-not-exist --output-format json` exited `124` with `stdout=0` and `stderr=0`. In the same rebuilt binary, `mcp list --output-format json` returned promptly, proving the MCP top-level surface itself is reachable; the hang is specific to the detail path for a missing server name. This is distinct from #327's MCP help/source-list mismatch and the earlier invalid #2874 hang hypothesis: the pinpoint is not help discovery, but missing-server detail lookup lacking a bounded typed `server_not_found` JSON response. **Required fix shape:** (a) make `mcp show <server> --output-format json` perform a bounded config lookup and return `kind:"error"` or equivalent with `code:"mcp_server_not_found"`, `server`, and optionally `available_servers[]` / count when the name is absent; (b) avoid attempting to launch/connect to an MCP transport before confirming the server exists in config; (c) emit stdout JSON for the primary error envelope and reserve stderr for diagnostics only; (d) add regression coverage proving a missing MCP server returns a typed JSON not-found response within a deterministic budget. **Why this matters:** MCP server inspection is a lifecycle diagnostic path. If a typo or stale server name can silently hang with no bytes, claws cannot tell whether the server is missing, config discovery stalled, transport launch blocked, or the CLI dispatcher deadlocked. Source: gaebal-gajae dogfood follow-up for the 20:30 nudge on rebuilt `./rust/target/debug/claw` `ee41b266`.
