...
356. **Top-level `status --help --output-format json` hangs with zero stdout/stderr instead of returning bounded command help JSON** — dogfooded 2026-04-30 for the 01:00 nudge on current `origin/main` / rebuilt `./rust/target/debug/claw` with embedded `git_sha` `74338dc6`. After rebuilding and verifying the binary provenance, repeated bounded runs of `timeout 8 ./rust/target/debug/claw status --help --output-format json` exited `124` with `stdout=0` and `stderr=0`. In the same rebuilt binary, `version --output-format json` returned promptly with version/build metadata, proving the binary itself and JSON output path are reachable; the hang is specific to the status help surface. This is distinct from #354/#355 memory/session JSON help/list hangs: the affected surface is the status command's help path, which should be local/static and should never wait on workspace inspection, credentials, or runtime status probes. **Required fix shape:** (a) make `status --help --output-format json` return static/bounded stdout JSON with `kind:"help"` or `kind:"status"`, `action:"help"`, usage, options, examples, and supported output formats; (b) ensure help rendering does not initialize slow status/workspace/runtime probes; (c) return a typed JSON timeout/unavailable error if any dynamic provider is accidentally consulted; (d) add regression coverage proving status help in JSON mode returns within a deterministic budget and emits no zero-byte hang. **Why this matters:** help must be the safe fallback when command behavior is unclear. If even `status --help --output-format json` can hang silently, claws cannot discover status semantics or recover from status-command failures without killing the process. Source: gaebal-gajae dogfood follow-up for the 01:00 nudge on rebuilt `./rust/target/debug/claw` `74338dc6`.
