...
389. **Top-level `acp --help --output-format json` exits successfully but emits plain text help instead of JSON** — dogfooded 2026-04-30 for the 07:00 nudge on current `origin/main` / local workspace while the checked-out `./rust/target/debug/claw` binary still reported embedded `git_sha` `d95b230c`. A fresh probe on the ACP/Zed discoverability surface showed a prompt-but-wrong contract: `./rust/target/debug/claw acp --help --output-format json` exited `0` with `stdout=461` and `stderr=0`, but stdout began with plain text `ACP / Zed`, `Usage`, `Aliases`, and `Purpose`; attempting to parse stdout as JSON failed at byte 0. In the same workspace, `version --output-format json` returned promptly with version/build metadata, proving the binary itself and JSON output path are reachable. This is distinct from the completed ACP status/discoverability item (#64a), the real ACP daemon follow-up (#76), zero-byte help hangs (#358/#380/#381/#382/#383/#387), and plaintext help fallbacks for export/init/sandbox (#384/#385/#386): the affected surface is help/discovery for ACP/Zed integration itself, where editor tooling and claws need machine-readable status/alias/serve-contract metadata. **Required fix shape:** (a) make `acp --help --output-format json` emit valid stdout JSON with `kind:"help"` or `kind:"acp"`, `action:"help"`, usage, aliases (`claw --acp`, `claw -acp`), current `serve` semantics, daemon availability, output formats, examples, and related roadmap/status fields; (b) preserve text help for default/text mode only; (c) add a `format:"json"` or equivalent field so callers can assert the contract without parsing prose; (d) add regression coverage proving ACP help with JSON format parses as JSON and stays consistent with `claw acp --output-format json`. **Why this matters:** ACP/Zed support is explicitly discoverability-sensitive. If ACP help accepts JSON mode but returns prose, editor integrations and claws cannot programmatically determine whether `serve` is a real daemon, a status alias, or unavailable without scraping human text. Source: gaebal-gajae dogfood follow-up for the 07:00 nudge on rebuilt/local `./rust/target/debug/claw` `d95b230c`.
