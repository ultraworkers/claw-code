...
386. **Top-level `sandbox --help --output-format json` exits successfully but emits plain text help instead of JSON** — dogfooded 2026-04-30 for the 05:30 nudge on current `origin/main` / local workspace while the checked-out `./rust/target/debug/claw` binary still reported embedded `git_sha` `d95b230c`. A fresh probe on the sandbox/preflight surface showed a prompt-but-wrong contract: `./rust/target/debug/claw sandbox --help --output-format json` exited `0` with `stdout=316` and `stderr=0`, but stdout began with plain text `Sandbox`, `Usage`, `Purpose`, `Output`, `Formats`, and `Related`; attempting to parse stdout as JSON failed at byte 0. In the same workspace, `version --output-format json` returned promptly with version/build metadata, proving the binary itself and JSON output path are reachable. This is distinct from zero-byte help hangs (#358/#380/#381/#382/#383), export plaintext help (#384), init plaintext help (#385), and the older sandbox runtime-state ambiguity items: the affected surface is help/discovery for `sandbox`, where claws need machine-readable semantics before trusting sandbox/isolation status fields. **Required fix shape:** (a) make `sandbox --help --output-format json` emit valid stdout JSON with `kind:"help"` or `kind:"sandbox"`, `action:"help"`, usage, options, output formats, examples, platform/fallback notes, and related slash/direct commands; (b) preserve text help for default/text mode only; (c) add a `format:"json"` or equivalent field so callers can assert the contract without parsing prose; (d) add regression coverage proving sandbox help with JSON format parses as JSON and does not silently fall back to plain text. **Why this matters:** sandbox status is a safety/control-plane contract. If sandbox help accepts JSON mode but returns prose, automation cannot discover what isolation fields mean or how fallback/degraded states are represented without scraping human text, making safe execution policy brittle. Source: gaebal-gajae dogfood follow-up for the 05:30 nudge on rebuilt/local `./rust/target/debug/claw` `d95b230c`.
