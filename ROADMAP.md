...
358. **Top-level `cost --help --output-format json` exits successfully but emits plain text help instead of JSON** — dogfooded 2026-04-30 for the 02:00 nudge on current `origin/main` / rebuilt `./rust/target/debug/claw` with embedded `git_sha` `d95b230c`. After rebuilding and verifying the binary provenance, repeated bounded runs of `./rust/target/debug/claw cost --help --output-format json` exited `0` with stdout help text and no stderr; the output starts with `Usage: claw cost [--output-format <format>]`, not a JSON object. In the same rebuilt binary, `doctor --help --output-format json` and `status --help --output-format json` also returned promptly as plain text (#357/#356), confirming a broader help-format fallback class while keeping this pinpoint on the cost surface. **Required fix shape:** (a) make `cost --help --output-format json` emit valid stdout JSON with `kind:"help"` or `kind:"cost"`, `action:"help"`, usage, options, examples, supported output formats, and related slash/direct commands; (b) preserve text help for default/text mode only; (c) add a `format:"json"` or equivalent field so callers can assert the contract without parsing prose; (d) add regression coverage proving cost help with JSON format parses as JSON and does not silently fall back to plain text. **Why this matters:** cost/tokens surfaces are commonly consumed by automation for budgeting. If JSON help falls back to prose, claws cannot discover cost command semantics without scraping formatting-sensitive text, and help stops being a safe machine-readable contract. Source: gaebal-gajae dogfood follow-up for the 02:00 nudge on rebuilt `./rust/target/debug/claw` `d95b230c`.
