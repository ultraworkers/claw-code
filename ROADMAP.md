...
357. **Top-level `doctor --help --output-format json` hangs with zero stdout/stderr instead of returning bounded command help JSON** — dogfooded 2026-04-30 for the 01:30 nudge on current `origin/main` / rebuilt `./rust/target/debug/claw` with embedded `git_sha` `52a909ce`. After rebuilding and verifying the binary provenance, repeated bounded runs of `timeout 8 ./rust/target/debug/claw doctor --help --output-format json` exited `124` with `stdout=0` and `stderr=0`. In the same rebuilt binary, `status --help --output-format json` returned promptly (though #356 records that it is plain text rather than JSON), so the binary is responsive and at least some help surfaces return; the hang is specific to the doctor help path. This is distinct from #356's status help plain-text fallback: doctor help never returns any bytes under the same JSON-mode help pattern. **Required fix shape:** (a) make `doctor --help --output-format json` return static/bounded stdout JSON with `kind:"help"` or `kind:"doctor"`, `action:"help"`, usage, checks, options, examples, supported output formats, and related slash/direct commands; (b) ensure help rendering does not initialize slow doctor probes or environment checks; (c) if any dynamic provider is accidentally consulted, return a typed JSON timeout/unavailable error instead of hanging; (d) add regression coverage proving doctor help in JSON mode returns within a deterministic budget. **Why this matters:** doctor is the diagnostic entrypoint users reach for when things are broken. If `doctor --help --output-format json` itself hangs silently, claws cannot discover diagnostic semantics or present safe recovery instructions before running potentially slow checks. Source: gaebal-gajae dogfood follow-up for the 01:30 nudge on rebuilt `./rust/target/debug/claw` `52a909ce`.
