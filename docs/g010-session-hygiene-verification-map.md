...
| --- | --- | --- |
| Generated session files are not accidentally committed | `.gitignore`, `rust/.gitignore` ignore `.brewcode/sessions/` and `.claude/sessions/` | `git check-ignore .brewcode/sessions/example.jsonl rust/.brewcode/sessions/example.jsonl .claude/sessions/example.json` |
...
- Runtime-owned session state remains under ignored `.brewcode/sessions/<workspace-fingerprint>/` paths.
- Resume-safe JSON output uses stable `kind` fields (`restored`, `compact`, `session_list`, `session_exists`, etc.) so brewcodes can route without scraping text.
