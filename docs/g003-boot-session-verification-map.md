...
- `rust/crates/runtime/src/worker_boot.rs`
  ...
  - File observability surface: `emit_state_file` writes `.brewcode/worker-state.json` with status, readiness, trust state, prompt-in-flight flag, last event, and update age.
