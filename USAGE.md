...
This guide covers the current Rust workspace under `rust/` and the `brewcode` CLI binary. If you are brand new, make the doctor health check your first run: start `brewcode`, then run `/doctor`.
...
`/doctor` is the built-in setup and preflight diagnostic. Once you have a saved session, you can rerun it with `./target/debug/brewcode --resume latest /doctor`.
...
The CLI binary is available at `rust/target/debug/brewcode` after a debug build (`rust\target\debug\brewcode.exe` on Windows). Make the doctor check above your first post-build step. For PowerShell-first install, release ZIP, PATH, provider-switching, and Windows/WSL notification examples, see [`docs/windows-install-release.md`](./docs/windows-install-release.md).
...
```bash
cd rust
./target/debug/brewcode doctor --output-format json
```
...
Set up a new repository with `.brewcode` config, `.brewcode.json`, `.gitignore` entries, and a `CLAUDE.md` guidance file:

```bash
cd /path/to/your/repo
./target/debug/brewcode init
```
...
```bash
./target/debug/brewcode init --output-format json
```
...
**Why structured fields matter:** Brewcodes can detect per-artifact state (`created` vs `updated` vs `skipped`) without substring-matching human prose. Use the `created[]`, `updated[]`, and `skipped[]` arrays for conditional follow-up logic (e.g., only commit if files were actually created, not just updated).
...
```bash
cd rust
./target/debug/brewcode prompt "summarize this repository"
```
...
```bash
cd rust
./target/debug/brewcode "explain rust/crates/runtime/src/lib.rs"
```
...
```bash
cd rust
./target/debug/brewcode --output-format json prompt "status"
```

### Inspect worker state

The `brewcode state` command reads `.brewcode/worker-state.json`, which is written by the interactive REPL or a one-shot prompt when a worker executes a task. This file contains the worker ID, session reference, model, and permission mode.

Prerequisite: You must run `brewcode` (interactive REPL) or `brewcode prompt <text>` at least once in the repository to produce the worker state file.

```bash
cd rust
./target/debug/brewcode state
```

JSON mode:
```bash
./target/debug/brewcode state --output-format json
```

If you run `brewcode state` before any worker has executed, you will see a helpful error:
```
error: no worker state file found at .brewcode/worker-state.json
...
These commands are available inside the interactive REPL (`brewcode` with no args). They extend the assistant with workspace analysis, planning, and navigation features.
...
```bash
cd rust
./target/debug/brewcode --model sonnet prompt "review this diff"
./target/debug/brewcode --permission-mode read-only prompt "summarize Cargo.toml"
./target/debug/brewcode --permission-mode workspace-write prompt "update README.md"
./target/debug/brewcode --allowedTools read,glob "inspect the runtime crate"
```
...
`brewcode` accepts two Anthropic credential env vars and they are **not interchangeable** — the HTTP header Anthropic expects differs per credential shape. Putting the wrong value in the wrong slot is the most common 401 we see.
...
**Why this matters:** if you paste an `sk-ant-*` key into `ANTHROPIC_AUTH_TOKEN`, Anthropic's API will return `401 Invalid bearer token` because `sk-ant-*` keys are rejected over the Bearer header. The fix is a one-line env var swap — move the key to `ANTHROPIC_API_KEY`. Recent `brewcode` builds detect this exact shape (401 + `sk-ant-*` in the Bearer slot) and append a hint to the error message pointing at the fix.

**If you meant a different provider:** if `brewcode` reports missing Anthropic credentials but you already have `OPENAI_API_KEY`, `XAI_API_KEY`, or `DASHSCOPE_API_KEY` exported, you most likely forgot to prefix the model name with the provider's routing prefix. Use `--model openai/gpt-4.1-mini` (OpenAI-compat / OpenRouter / Ollama), `--model grok` (xAI), or `--model qwen-plus` (DashScope) and the prefix router will select the right backend regardless of the ambient credentials. The error message now includes a hint that names the detected env var.
...
`CLAUDE_CODE_PROVIDER` is not required for normal Brewcode routing; prefer explicit model prefixes such as `openai/` and provider-specific env vars so PowerShell examples stay portable.
...
cd rust
./target/debug/brewcode --model "claude-sonnet-4-6" prompt "reply with the word ready"
```
...
cd rust
./target/debug/brewcode --model "qwen2.5-coder" prompt "reply with the word ready"
```
...
cd rust
./target/debug/brewcode --model "llama3.2" prompt "summarize this repository in one sentence"
```
...
cd rust
./target/debug/brewcode --model "openai/gpt-4.1-mini" prompt "summarize this repository in one sentence"
```
...
cd rust
./target/debug/brewcode --model "qwen/qwen-max" prompt "hello"
# or bare:
./target/debug/brewcode --model "qwen-plus" prompt "hello"
```
...
`brewcode` has three built-in provider backends. The provider is selected automatically based on the model name, falling back to whichever credential is present in the environment.
...
You can add custom aliases in any settings file (`~/.brewcode/settings.json`, `.brewcode/settings.json`, or `.brewcode/settings.local.json`):
...
6. Otherwise, `brewcode` checks which credential is set: Anthropic first, then OpenAI, then xAI. If only `OPENAI_BASE_URL` is set, it still routes to OpenAI-compatible for authless local servers.
...
Use `@path/to/file` in prompts to submit repository files as context, for example `Read @src/app.ts and explain the bug`, `Compare @old.md and @new.md`, or `Use @logs/error.txt as context and suggest a fix`. Prompt history, `Ctrl-r`, and long-output scrolling come from your shell, terminal, or tmux rather than from Brewcode itself. See [`docs/navigation-file-context.md`](./docs/navigation-file-context.md) for scrollback, attachment, and secret-redaction guidance.

## FAQ

### Is Brew Code Claude-only?

No. Brew Code is a Claude-Code-shaped workflow/runtime, not a Claude-only product. It can target Anthropic and OpenAI-compatible/provider-routed/local models depending on config. Non-Claude providers may require stricter response-shape and tool-call compatibility, so some workflows can be rougher than first-party Anthropic/OpenAI paths; provider-specific identity leaks are bugs, not product intent. See [`docs/local-openai-compatible-providers.md`](./docs/local-openai-compatible-providers.md) for local provider examples.

### What about Codex?

The name "codex" appears in the Brew Code ecosystem but it does **not** refer to OpenAI Codex (the code-generation model). Here is what it means in this project:

- **`oh-my-codex` (OmX)** is the workflow and plugin layer that sits on top of `brewcode`. It provides planning modes, parallel multi-agent execution, notification routing, and other automation features. See [PHILOSOPHY.md](./PHILOSOPHY.md) and the [oh-my-codex repo](https://github.com/Yeachan-Heo/oh-my-codex).
- **`.codex/` directories** (e.g. `.codex/skills`, `.codex/agents`, `.codex/commands`) are legacy lookup paths that `brewcode` still scans alongside the primary `.brewcode/` directories.
- **`CODEX_HOME`** is an optional environment variable that points to a custom root for user-level skill and command lookups.

`brewcode` does **not** support OpenAI Codex sessions, the Codex CLI, or Codex session import/export. If you need to use OpenAI models (like GPT-4.1), configure the OpenAI-compatible provider as shown above in the [OpenAI-compatible endpoint](#openai-compatible-endpoint) and [OpenRouter](#openrouter) sections.

## HTTP proxy support

`brewcode` honours the standard `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` environment variables (both upper- and lower-case spellings are accepted) when issuing outbound requests to Anthropic, OpenAI-, and xAI-compatible endpoints. Set them before launching the CLI and the underlying `reqwest` client will be configured automatically.
...
cd rust
./target/debug/brewcode prompt "hello via the corporate proxy"
```
...
- If a proxy URL cannot be parsed, `brewcode` falls back to a direct (no-proxy) client so existing workflows keep working; double-check the URL if you expected the request to be tunnelled.

## Skills

Use `/skills list` in the interactive REPL or `brewcode skills --output-format json` from the direct CLI to inspect installed skills. For offline/local installs, install the directory that contains `SKILL.md`, then verify the discovered name before invoking it:
...
If install succeeds but invocation fails with a provider HTTP error, treat provider setup separately: run `brewcode doctor` and a one-shot prompt smoke test before reinstalling the skill. See [`docs/local-openai-compatible-providers.md`](./docs/local-openai-compatible-providers.md#local-skills-install-from-disk) for the full checklist.

## Common operational commands

```bash
cd rust
./target/debug/brewcode status
./target/debug/brewcode sandbox
./target/debug/brewcode agents
./target/debug/brewcode mcp
./target/debug/brewcode skills
./target/debug/brewcode system-prompt --cwd .. --date 2026-04-04
```

## Session management

REPL turns are persisted under `.brewcode/sessions/` in the current workspace.

```bash
cd rust
./target/debug/brewcode --resume latest
./target/debug/brewcode --resume latest /status /diff
```
...
1. `~/.brewcode.json`
2. `~/.config/brewcode/settings.json`
3. `<repo>/.brewcode.json`
4. `<repo>/.brewcode/settings.json`
5. `<repo>/.brewcode/settings.local.json`
