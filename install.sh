...
# builds the `brewcode` binary from the `rust/` workspace, and runs a
...
#   BREWCODE_BUILD_PROFILE=debug|release   same as --release toggle
#   BREWCODE_SKIP_VERIFY=1                 same as --no-verify
...
Environment overrides:
  BREWCODE_BUILD_PROFILE   debug | release
  BREWCODE_SKIP_VERIFY     set to 1 to skip verification
EOF
}
...
BUILD_PROFILE="${BREWCODE_BUILD_PROFILE:-debug}"
SKIP_VERIFY="${BREWCODE_SKIP_VERIFY:-0}"
...
${COLOR_DIM}---------------${COLOR_RESET}
  ...
  ${COLOR_BOLD}6. 'brewcode' not found after install${COLOR_RESET}
     The binary lives at:
       rust/target/${BUILD_PROFILE}/brewcode
...
step "Building the brewcode workspace (${BUILD_PROFILE})"
...
BREWCODE_BIN="${RUST_DIR}/target/${BUILD_PROFILE}/brewcode"

if [ ! -x "${BREWCODE_BIN}" ]; then
    error "Expected binary not found at ${BREWCODE_BIN}"
    error "The build reported success but the binary is missing — check cargo output above."
    exit 1
fi

ok "built ${BREWCODE_BIN}"
...
if [ "${SKIP_VERIFY}" = "1" ]; then
    warn "verification skipped (--no-verify or BREWCODE_SKIP_VERIFY=1)"
else
    info "running: brewcode --version"
    if VERSION_OUT="$("${BREWCODE_BIN}" --version 2>&1)"; then
        ok "brewcode --version -> ${VERSION_OUT}"
    else
        error "brewcode --version failed:"
        printf '%s\n' "${VERSION_OUT}" 1>&2
        exit 1
    fi

    info "running: brewcode --help (smoke test)"
    if "${BREWCODE_BIN}" --help >/dev/null 2>&1; then
        ok "brewcode --help responded"
    else
        error "brewcode --help failed"
        exit 1
    fi
fi
...
cat <<EOF
${COLOR_GREEN}Brew Code is built and ready.${COLOR_RESET}

  Binary:  ${COLOR_BOLD}${BREWCODE_BIN}${COLOR_RESET}
...
Try it out:
  ...
  ${BREWCODE_BIN} prompt "summarize this repository"
